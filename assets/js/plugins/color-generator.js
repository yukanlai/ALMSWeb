	...
	function define_Please(){
		...
		Please.make_scheme = function( HSV, options ){
			...
			switch( scheme_options.scheme_type.toLowerCase() ){
				...
				break;
				case 'rainbow':
					for ( i = 1; i <= scheme_options.colors_returned; i++ ) {
						adjusted = clone( HSV );
						adjusted.h = ( adjusted.h + ( (360 / scheme_options.colors_returned - 1) * i ) ) % 360;
						adjusted.s -= .1;
						adjusted.v -= .1;
						scheme.push( adjusted );
					}
				...
				case 'half-rainbow':
					for ( i = 1; i <= scheme_options.colors_returned; i++ ) {
						adjusted = clone( HSV );
						adjusted.h = ( adjusted.h + ( (180 / scheme_options.colors_returned - 1) * i ) ) % 360;
						adjusted.s -= .1;
						adjusted.v -= .1;
						scheme.push( adjusted );
					}
